...
import {
  ...
  isTabList,
  isTabPanel,
} from "../Common/Helpers/Tabs/elementTypes";
...
function determineCanUseActiveElement(environment: Window | null | undefined) {
  ...
  try {
    canUseActiveElement = !!(env && env.document && env.document.activeElement);
  } catch (e) {
...
export const UncontrolledTabs: FunctionComponent<TabProps> = (props) => {
  ...
  function getChildren() {
    let index = 0;
    const { children, focus, forceRenderTabPanel, selectedIndex, environment } =
      props;
