  ...
  return (
    <SidebarItemElement
      className={[
        "Base__SidebarItem",
        "SidebarItem",
        active ? "Base__SidebarItem--Activated SidebarItemActive" : "",
        className,
      ].join(" ")}
      onClick={onClick}
      {...props}
    >
      <IconElement className="material-symbols-rounded">{icon}</IconElement>

      {!disableText && (
        <div className={"Base__SidebarItem--Text"}>{text || children}</div>
      )}
    </SidebarItemElement>
  );
}
